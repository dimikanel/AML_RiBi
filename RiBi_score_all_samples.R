...
# Load clinical metadata. Discard APL and NA samples / keep all samples (i.e. refractory etc)
...
# Load the metadata, rename, keep all samples with available RNASeq data 

finn_annot <- read.csv('File_0_Common_sample_annotation_252S.csv') %>%
  separate(Sample_ID, c('patient_ID_1', 'rep'), sep = '_(?=[^_]+$)', remove = FALSE) %>%
  filter(RNA.seq == 'available') %>% filter(Sample_ID %in% colnames(finn_counts)) %>% 
...
# Drop Healthy samples and normalize with DeSeq2

meta_all <- meta_all %>% filter(!Disease_Status == 'Healthy')
all_counts <- all_counts %>% select(meta_all$patient)
...
# Load the df with gene signature and their coefficients

ribi_sign <- read.csv('RiBi_model_coefficients.csv') 


# Subset Ribi genes and calculate the RiBi score

selected_genes <- intersect(ribi_sign$gene, rownames(norm_counts))
expr_matrix <- norm_counts[selected_genes, , drop = FALSE]
coeff_vector <- ribi_sign$coefficient[match(rownames(expr_matrix), ribi_sign$gene)]
expr_matrix_t <- t(expr_matrix)  
weighted_matrix <- sweep(expr_matrix_t, 2, coeff_vector, `*`)
score_df <- data.frame(patient = rownames(expr_matrix_t), RiBi_score = rowSums(weighted_matrix, na.rm = TRUE))
...
write.csv(meta_with_score, 'RiBi_score_all_samples.csv')


# calculate LSC17 score and correlate with RiBi score
...
LSC17_df <- read.csv('LSC17.csv', header = T, sep = ',')
colnames(LSC17_df)[1] <- 'external_gene_name'
genes <- LSC17_df$external_gene_name


gene.list <- getBM(filters= 'external_gene_name',
...
LSC17_df <- merge(LSC17_df, gene.list, by = 'external_gene_name', all = T)


selected_genes_LSC17 <- intersect(LSC17_df$ensembl_gene_id, rownames(norm_counts))
expr_matrix_LSC17 <- norm_counts[selected_genes_LSC17, , drop = FALSE]
coeff_vector_LSC17 <- LSC17_df$Weight[match(rownames(expr_matrix_LSC17), LSC17_df$ensembl_gene_id)]
expr_matrix_t_LSC17 <- t(expr_matrix_LSC17)  
weighted_matrix_LSC17 <- sweep(expr_matrix_t_LSC17, 2, coeff_vector_LSC17, `*`)
score_df_LSC17 <- data.frame(patient = rownames(expr_matrix_t_LSC17), LSC17_score = rowSums(weighted_matrix_LSC17, na.rm = TRUE))


# Classify score into quantile-based risk groups

score_df_LSC17 <- score_df_LSC17 %>%
  mutate(LSC17_status = case_when(
    LSC17_score > quantile(LSC17_score, 0.75) ~ 'high',
    LSC17_score < quantile(LSC17_score, 0.25) ~ 'low',
    TRUE ~ 'moderate'
  ))
...
meta_with_score <- meta_with_score %>%
  left_join(score_df_LSC17, by = 'patient') %>%
  filter(!is.na(LSC17_score), time > 0)


write.csv(meta_with_score, 'RiBi_score_LSC17_score_all_samples.csv')


# correlate RiBi and LSC17 score

ggscatter(meta_with_score, y = 'LSC17_score', x = 'RiBi_score',
          add = 'reg.line',                                 
          conf.int = TRUE,                                  
